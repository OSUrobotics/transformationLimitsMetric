...
% USAGE
%
%       [ transformationFP, objectFP, surfaceFP, handFP, outputFP ] = filenamesFromComponents( objectNum, subjectNum, graspNum, extreme, stepNum )
%
% INPUTS
%
...
%       >>  [ transformationFP, objectFP, surfaceFP, handFP, outputFP ] = filenamesFromComponents( 7, 9, 2, 'extreme0_prime_', 1:9 )
%
%==========================================================================
...
if nargin == 5 && length(stepNum) ~= 1
    ...
    for stepNumIndex = 1:length(stepNum)
        outputFP{stepNumIndex} = sprintf('Output/Step%iobj%i_sub%i_grasp%i_%sAreaIntersection.csv',stepNum(stepNumIndex),objectNum,subjectNum,graspNum,extreme);
    end
elseif nargin == 5
    outputFP = sprintf('Output/Step%iobj%i_sub%i_grasp%i_%sAreaIntersection.csv',stepNum,objectNum,subjectNum,graspNum,extreme);
else
    outputFP = sprintf('Output/Step%sobj%i_sub%i_grasp%i_%sAreaIntersection.csv','%i',objectNum,subjectNum,graspNum,extreme);
end
