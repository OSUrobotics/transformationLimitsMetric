...
%% WIGGLE WIGGLE WIGGLE Generating the transforms here
wiggleStructAndFile = 'wiggleStored';
wiggleSettings = struct;
wiggleSettings.translateScalar = 0.2;
wiggleSettings.numTranslationDirections = 12;
wiggleSettings.angleDivisions = [-1 -.5 -.25 0 .25 .5 1];
%% Other variables
...
%% If not already loaded, load the WIGGLE values
if ~exist('transformationStruct','var')
    %% If not already created, create the file
    if ~exist(wiggleStructAndFile,'file')
        disp('Started generating WIGGLE');
        wiggleStruct = saveTrajectories(wiggleSettings,wiggleStructAndFile);
        disp('Done generating WIGGLE');
    else
        load(wiggleStructAndFile);
        disp('Loaded WIGGLE');
    end
else
    disp('Using preloaded WIGGLE');
end
